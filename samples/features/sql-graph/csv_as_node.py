...
def main(input_file_path, schema, table):
    ...
    output_file_path = os.path.splitext(
        input_file_path)[0] + "_as_node" + ".csv"

    # you may have to change the encoding
    with open(input_file_path, mode="r", encoding="utf-16le",
              newline="") as input_file, open(output_file_path,
                                              mode="w",
                                              encoding="utf-16le",
        ...
        while line:
            if line_number == 0:
                newline = ("\ufeff" + '{"type":"node","schema":"' + schema +
                           '","table":"' + table + '","id":' +
                           str(line_number) + "}\t" + line[1:])
            else:
                newline = ('{"type":"node","schema":"' + schema +
                           '","table":"' + table + '","id":' +
